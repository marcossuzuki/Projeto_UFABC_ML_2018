...
def evolve(population, retain_lenght=0.4, random_select=0.1, max_pop=10, number_gen=1):
    ...
    for i, element in enumerate(pop_gen):
        element.save("./save3/marioGA_3.gen_"+str(number_gen)+"ind"+str(i))
...
def generations(pop, n_gen=10):
    ...
    for i in range(n_gen):
        new_pop = evolve(initial_population, random_select=0, number_gen = i)
        initial_population = new_pop
    return new_pop

def main():
    initial_population = population(30, state_size, action_size)
    '''
    for i in range(4):
