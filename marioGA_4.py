...
def evolve(population, retain_lenght=0.4, random_select=0.1, max_pop=5, number_gen=1):
...
def mutate(weights, x):
    for xi in range(x-400,len(weights[2])):
        try:
...
def main():
    ...
    for i in range(1):
        agent = DQNAgent(state_size, action_size)
        agent.load("./save4/marioGA_4.ind" +str(0))
        fit, x = fitness(agent)
        initial_population.append((fit, agent, x))
        agent = DQNAgent(state_size, action_size)
        agent.load("./save4/melhores resultados/marioGA_4.ind" +str(3))
        fit, x = fitness(agent)
        initial_population.append((fit, agent, x))
    for i in range(5):
        agent = DQNAgent(state_size, action_size, no_ones_initializer=True)
        fit, x = fitness(agent)
        initial_population.append((fit, agent, x))
